...
import { Box, Card, CardContent, Typography, Chip, IconButton, Collapse, Paper, Divider } from "@mui/material";
import {
...
export default function PatchesPanel({ patches }: Props) {
        ...
        <Box>
            ...
            :   <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    {sortedPatches.map((patch, index) => (
                        <PatchCard key={`${patch.baseCommit}-${patch.userName}-${index}`} patch={patch} />
                    ))}
                </Box>
            }
        </Box>
