...
import { FileActivityTracker, FileActivity } from "./fileActivityTracker";
...
type TreeItemKind = "repository" | "user" | "directory" | "file" | "activity" | "placeholder";
...
export class ActivityTreeDataProvider implements vscode.TreeDataProvider<WorkShareTreeItem> {
    ...
    private async getScopedActivities(): Promise<RepositoryScope> {
        ...
        return {
            repositoryRemoteUrl,
            activities: this.mergeActivities(localActivities, serverActivities),
        };
    }
    ...
    getChildren(element?: WorkShareTreeItem): Thenable<WorkShareTreeItem[]> {
        ...
        if (element.kind === "user") {
            ...
            const fileItems = Array.from(directoryTree.files.entries())
                .sort(([left], [right]) => left.localeCompare(right))
                .map(([fileName, fileActivities]) => WorkShareTreeItem.file(fileName, fileActivities));
...
class WorkShareTreeItem extends vscode.TreeItem {
    ...
    static repository(label: string, repositoryRemoteUrl: string | undefined, activities: FileActivity[]): WorkShareTreeItem {
        const item = new WorkShareTreeItem("repository", label, vscode.TreeItemCollapsibleState.Expanded, repositoryRemoteUrl, activities);
    ...
    static user(userName: string, activities: FileActivity[]): WorkShareTreeItem {
        const item = new WorkShareTreeItem("user", userName, vscode.TreeItemCollapsibleState.Collapsed, undefined, activities);
    ...
    static directory(directoryName: string, directoryTree: DirectoryTree): WorkShareTreeItem {
        const item = new WorkShareTreeItem("directory", directoryName, vscode.TreeItemCollapsibleState.Collapsed, undefined, undefined, directoryTree);
        item.iconPath = new vscode.ThemeIcon("folder");
        return item;
    }

    static file(fileName: string, activities: FileActivity[]): WorkShareTreeItem {
        const item = new WorkShareTreeItem("file", fileName, vscode.TreeItemCollapsibleState.Collapsed, undefined, activities);
        item.iconPath = new vscode.ThemeIcon("file");
        item.description = `${activities.length} events`;
        item.tooltip = `${fileName} has ${activities.length} tracked events.`;
        return item;
    }

    static activity(activity: FileActivity): WorkShareTreeItem {
        const label = `${activity.action} • ${activity.timestamp.toLocaleString()}`;
        const item = new WorkShareTreeItem("activity", label, vscode.TreeItemCollapsibleState.None, undefined, undefined, undefined, activity);
